def stack(spec, red, trunc, s2n, dv):
    ...
    # for s2n of 2.5, use trunc = [0,935]
...
# truncating each spectra to only include wavelength values from 950A to 1500A
    ...
    for i in r:
        t_spec.append(XSpectrum1D(norm_spec[i].wavelength[trim[i]][trunc],
                                  norm_spec[i].flux[trim[i]][trunc],
                                  norm_spec[i].flux[trim[i]][trunc]))
