...
norm = median(s.flux[where(s.wave gt 1267 and s.wave lt 1276)])
...
; (hint: metallicity changes every 10 models so 
...
ISM_lines = [1260.42, 1302.17, 1304.37, 1335.71, 1393.76, 1402.77, 1526.71, 1548.19, 1550.77] ;These are the strong ISM absorption lines between 1230-1600A.
;They are Si II 1260, O I1 1302, Si II 1304, C II 1335, Si IV 1394, Si IV 1402, Si II 1526, C IV 1548/1550.
...
mask = fltarr(n_elements(s.wave))+1.


;Now we will go through and mark the mask array with a 0 if we do not want to include it.
for maskn = 0, n_lines-1 do begin & $
  tempmask = where(s.wave gt -dv/cspeed*ISM_lines[maskn]+ISM_lines[maskn] and s.wave lt dv/cspeed*ISM_lines[maskn]+ISM_lines[maskn])   & $ ;This excludes +/-500km/s around each line
  if ISM_lines[maskn] eq 1550.77 then tempmask = where(s.wave gt -dv/cspeed*ISM_lines[maskn]+ISM_lines[maskn] and s.wave lt 50./cspeed*ISM_lines[maskn]+ISM_lines[maskn])   & $ ; CIV has a special stellar feature that we want redward of line center
...
good_reg = where(s.wave gt 1225 and s.wave lt 1600 and mask eq 1 and s.flux/s.err gt 1, comp = bad_reg)
...
plot, s.wave, s.flux, xr = [1240, 1600], yr = [0,2], xtit = 'Rest Wavelength', ytit = 'Normalized flux'
;djs_oplot, s.wave[bad_reg], s.flux[bad_reg], color = 'gray'
;If you see narrow absorption lines in black and not in gray go back and add them to the ISM_lines call on line 76.


vdisp = 275. ; FWHM of spectral resolution of the observations in km/s
...
; 
cont_params = s99_continuum(models, s.wave, s.flux,$
  s.err, good_reg, vdisp, yfit = cont_fit)
help, cont_params, /st
...
plot, s.wave, s.flux, yr = [0, 2], xr = [1200, 1600], /xs, xtitle = 'Wavelength', $
ytitle = 'Normalized Flux', charthick = 4
;djs_oplot, s.wave[bad_reg], s.flux[bad_reg], color = 'gray'
;djs_oplot, s.wave, s.err, color = 'light blue', thick = 3 ;this is the error on the flux
;djs_oplot, s.wave, cont_fit, color = 'light red', thick = 3; this is the stellar continuum fit
