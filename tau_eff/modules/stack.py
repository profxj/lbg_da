def lbg_smash(spec, red, ebv, s2n, dv):
    ...
    :return: a single composite XSpectrum1D object, the spectra that compose it
    ...
    lylim = [(entry < 912) for entry in temp]

    lylim_flux = np.asarray([spec[i].flux[lylim[i]] for i in r])

    lylim_cut = [all(i[:]!= 0.0) for i in lylim_flux]
    ...
    print("the median redshift value z of your sample =", z)

    print("the RMS of the redshift bin =", z_sig)
    ...
    if len(fix) > 0:
        ...
        #bad = np.array([10,14,20,23,26,28]) # for the 2.5<z<2.75 bin
        #good_spec = np.delete(good_spec,bad)

        collate = ltsu.collate(good_spec)

        n = collate.nspec

        print("Number of spectra to be stacked =", n)
